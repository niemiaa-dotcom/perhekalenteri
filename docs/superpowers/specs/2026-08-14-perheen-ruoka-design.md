...
## Osa 2: Hellä varainen viikkosuunnittelija (Hermes-puoli)
...
## Osa 3: Älykäs ostoslista (Hermes + sovelluksen storage)

Käyttää olemassa olevaa `shopping_list`-kokoelmaa ja `/api/shopping`-endpointteja.

**Automaattinen kokoaminen:** hyväksytyn suunnitelman ainekset kerätään yhdeksi
kauppalistaksi (`POST /api/shopping` per tuote).

**Älykäs käsittely (minä):**
1. **Duplikaattien yhdistäminen** — sama tuote useasta reseptistä yhdistetään
   yhdeksi riviksi; määrät lasketaan yhteen (esim. "kerma 2 dl" + "kerma 1,5 dl"
   → "kerma 3,5 dl").
2. **Määrälaskenta annosmäärän mukaan.**
3. **Ruokavalio-korvaukset** — kun aines sisältää maitoa/laktoosia, listalle
   menee oikea korvike (esim. "maito" → "laktoositon maito" tai "kaurajuoma"
   Ainolle). Perustuu reseptin `notes` / `diet_tags`.
4. **Kauppakäytävä-lajittelu** — tuotteet ryhmiteltynä kaupan osastojen mukaan
   (maitotuotteet, leipä, vihannekset, liha/kala, pakaste, juomat, ...).
   Lajitteluryhmät sovelluksen UI:hin näkyväksi.
